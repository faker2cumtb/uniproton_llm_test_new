...
#elif defined(DEMO_I2C) || defined(DEMO_RTC)
    FUNC_CALL(i2c_ds1339_demo);
#elif defined(DEMO_SMP)
    FUNC_CALL(smp_demo);
#elif defined(DEMO_MAC)
    FUNC_CALL(mac_demo);
#elif defined(DEMO_LOCALBUS)
    FUNC_CALL(localbus_demo);
...
#error "OS_SUPPORT_NET not defined!"
...
#elif defined(DEMO_POWERDOWN_PROTECT)
    FUNC_CALL(demo_powerdown_protect);
    ...
    FUNC_CALL(log_demo);
#elif defined(DEMO_FPGA_INTR)
    FUNC_CALL(fpga_intr_demo);
#endif
}
