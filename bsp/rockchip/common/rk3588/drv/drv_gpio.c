 ...
 * Change Logs:
 * Date           Author       Notes
 * 2024-07-22     LuoYuncong   the first version
 */
#include <rtdevice.h>
#include <prt_hwi.h>
...
#define GPIO0_INT 309
#define GPIO1_INT 310
#define GPIO2_INT 311
#define GPIO3_INT 312
#define GPIO4_INT 313
...
struct pin_index *rk3588_get_pin(rt_uint8_t pin)
{
...
static void rk3588_pin_mode(struct rt_device *device, rt_base_t pin, rt_base_t mode)
{
    struct pin_index *index = rk3588_get_pin(pin);
    if (index == RT_NULL)
    {
        return;
    }
...
static void rk3588_pin_write(struct rt_device *device, rt_base_t pin, rt_base_t value)
{
    struct pin_index *index = rk3588_get_pin(pin);
    if (index == RT_NULL)
    {
...
static int rk3588_pin_read(struct rt_device *device, rt_base_t pin)
{
    int value = PIN_LOW;
    struct pin_index *index = rk3588_get_pin(pin);
...
static rt_err_t rk3588_pin_attach_irq(struct rt_device *device, rt_int32_t pin, rt_uint32_t mode, void (*hdr)(void *args), void *args)
{
    struct pin_index *index = rk3588_get_pin(pin);
...
static rt_err_t rk3588_pin_detach_irq(struct rt_device *device, rt_int32_t pin)
{
    struct pin_index *index = rk3588_get_pin(pin);
...
static void rk3588_pin_irq_handler(uintptr_t para)
{
    switch (para)
    {
    case 0:
...
static int rk3588_pin_irq_init(struct pin_index *index)
{
    U32 ret;
    ...
    ret = PRT_HwiCreate(index->irq_num, (HwiProcFunc)rk3588_pin_irq_handler, index->bank);
    if (ret != OS_OK)
    {
        return ret;
    }
...
static rt_err_t rk3588_pin_irq_enable(struct rt_device *device, rt_base_t pin, rt_uint32_t enabled)
{
    struct pin_index *index = rk3588_get_pin(pin);
    if (index == RT_NULL)
    {
...
static rt_base_t rk3588_pin_get(const char *name)
{
    if (strlen(name) != 5)
    {
        return -1;
...
const static struct rt_pin_ops _rk3588_pin_ops = {
    rk3588_pin_mode,
    rk3588_pin_write,
    rk3588_pin_read,
    rk3588_pin_attach_irq,
    rk3588_pin_detach_irq,
    rk3588_pin_irq_enable,
    rk3588_pin_get,
};
...
int drv_pin_init(void)
{
    ...
    return rt_device_pin_register("pin", &_rk3588_pin_ops, RT_NULL);
}
