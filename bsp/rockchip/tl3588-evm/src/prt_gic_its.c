...
#define GITS_BASE_ADDR              0xFE640000      // GITS基地址
#define GICR_TYPER_CPU_NUMBER(r)	(((r) >> 8UL) & 0xffff)
    ...
    its_fill_cmd_fn fill_cmd_fn, union its_cmd_arg_s *cmd_arg)
{
    ...
    cache_flush_by_range(cmd, GITS_CMD_ENTRY_SIZE);
...
static void its_map_device_id_to_itt(struct its_device_s *its_device, U32 device_id, U32 event_id_range)
{
    ...
    cache_flush_by_range(itt_addr, mem_size);
...
int its_map_icid_to_redist(struct its_device_s *its_device, struct redist_addr_info_s *gicr_addr, U32 collect_id)
{
 ...
 if (its_device->its_attr.pta) {
       target_addr = gicr_addr->phy_addr;
    }
    else{
        /* This ITS wants a linear CPU number. */ 
		target_addr = io_reg_read64(U64_to_ptr(gicr_addr->phy_addr, void *), GITS_TYPER);
		target_addr = GICR_TYPER_CPU_NUMBER(target_addr) << 16UL;
    }
    ...
    its_device->rd_phy[core_id] = target_addr;
...
static void *its_alloc_cmd_queue(void)
{
    ...
    cache_flush_by_range(cmd_queue, GITS_CMD_QUEUE_SIZE);
...
int its_alloc_its_tables(struct its_device_s *its_device)
{
    for (U32 i = 0U; i < GITS_BASER_REG_NR; i++) {
        ...
        // Only for no-cache setting
        its_baser &= ~((GITS_BASER_InnerShareable << GITS_CBASER_SHAREABILITY_SHIFT) |
                (GITS_CACHE_RaWaWb << GITS_CBASER_IN_CACHE_SHIFT));
        its_baser |= (GITS_CACHE_NCACHEABLE << GITS_CBASER_IN_CACHE_SHIFT);
...
int its_init_cmd_queue(struct its_device_s *its_device)
{
    ...
    // Only for no-cache setting
    its_cbaser &= ~((GITS_SHARE_InnerShareable << GITS_CBASER_SHAREABILITY_SHIFT) |
			(cache_ability << GITS_CBASER_IN_CACHE_SHIFT));
	its_cbaser |= (GITS_CACHE_NCACHEABLE << GITS_CBASER_IN_CACHE_SHIFT);
...
int its_alloc_lpi_prop_table(struct its_device_s *its_device)
{
    ...
    cache_flush_by_range(prop_table_addr, GITS_PROP_TABLE_SIZE);
...
int its_alloc_lpi_pend_table(struct its_device_s *its_device)
{
    for (U32 i = 0; i < OS_MAX_CORE_NUM; i++) {
        ...
        cache_flush_by_range(its_device->its_res.pending_table_addr[i], GITS_PEND_TABLE_SIZE);
    }
...
static void its_set_irq_enable_disable(struct its_device_s *its_device, U32 hwirq, bool enable)
{
    ...
    cache_flush_by_range(prop_table_entry, sizeof(U8));
    ...
    struct its_device_s *its_device)
{
    ...
    // Only for no-cache setting
    gicr_pendbaser &= ~((GITS_SHARE_InnerShareable << GICV3_GICR_PENDBASER_SHAREABILITY_SHIFT) |
			(GITS_CACHE_RaWaWb << GICV3_GICR_PENDBASER_CACHEABILITY_SHIFT));
	gicr_pendbaser |= (GITS_CACHE_NCACHEABLE << GICV3_GICR_PENDBASER_CACHEABILITY_SHIFT);
    ...
    // Only for no-cache setting
    gicr_pendbaser &= ~((GITS_SHARE_InnerShareable << GICV3_GICR_PROPBASER_SHAREABILITY_SHIFT) |
			(GITS_CACHE_RaWaWb << GICV3_GICR_PROPBASER_CACHEABILITY_SHIFT));
	gicr_pendbaser |= (GITS_CACHE_NCACHEABLE << GICV3_GICR_PROPBASER_CACHEABILITY_SHIFT);
