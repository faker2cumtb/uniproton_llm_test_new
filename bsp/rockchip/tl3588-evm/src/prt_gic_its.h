...
/*CBASER 相关*/
#define GITS_BASER_INDIRECT			    (1ULL << 62U)
...
/*ITS commands*/
#define GITS_CMD_ENTRY_SIZE             32U //(GITS_CMD_QUEUE_SIZE / (sizeof(U64) * 4UL)) //32U //(GITS_CMD_QUEUE_SIZE / (sizeof(U64) * 4))
...
/*CACHE属性*/
#define GITS_CACHE_NCACHEABLE	        1UL /* Non-cacheable */
...
static inline void io_reg_write64(void *base_addr, unsigned long offset, U64 val)
{
    ...
    *(volatile uint64_t *)(base_addr + offset) = val;
}
