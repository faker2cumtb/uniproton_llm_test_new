...
function build_mbedtls() {
    echo "###################### build mbedtls start ######################"
    local msge="####################### build mbedtls end #######################"

    local mbedtls_dir=$HOME_DIR/components/security/mbedtls-3.6.2
    local output_dir=$BUILD_DIR/components/security/mbedtls

    pushd "$mbedtls_dir" >/dev/null || exit 1

    # clean old files
    rm -f "$BOARD_DIR/libs/$COMPILE_MODE/libmbedtls.a"
    # rm -fr "$BOARD_DIR/include/mbedtls"
    rm -fr "$output_dir"
    ...
    if [ $rc_make -ne 0 ]; then
        echo "ERROR: make mbedtls failed!" >&2
        popd >/dev/null
        exit 1
    fi
