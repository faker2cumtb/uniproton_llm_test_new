...
def psftp_get(conn, remote_dir, remote_file="", local_dir=os.getcwd()):
    ...
    if ssh_cmd.pssh_cmd(conn, "test -d " + remote_dir)[0]:
        logger.error("remote dir:%s does not exist" % remote_dir)
        conn.close()
        sys.exit(1)
    ...
    if remote_file == "":
        ...
        if ssh_cmd.pssh_cmd(conn, "test -f " + os.path.join(remote_dir, remote_file))[0]:
            logger.error("remote file:%s does not exist" % remote_file)
            conn.close()
            sys.exit(1)
    ...
    for f in all_file:
        ...
        logger.info("start to get file:%s......" % f)
...
def psftp_put(conn, local_dir=os.getcwd(), local_file="", remote_dir=""):
    ...
    if subprocess.getstatusoutput("test -d " + local_dir)[0]:
        logger.error("local dir:%s does not exist" % local_dir)
        conn.close()
        sys.exit(1)
    ...
    if local_file == "":
        ...
        if subprocess.getstatusoutput("test -f " + os.path.join(local_dir, local_file))[0]:
            logger.error("local file:%s does not exist" % local_file)
            conn.close()
            sys.exit(1)
    ...
    for f in all_file:
        ...
        logger.info("start to put file:%s......" % f)

    conn.close()

def psftp_put_simple(conn, local_file="", remote_dir=""):
    """将本地文件传输到远端
    ...
    sftp = paramiko.SFTPClient.from_transport(conn.get_transport())
    logger.info("start to put file:%s......" % local_file)
    print(local_file, remote_dir)
...
if __name__ == "__main__":
    ...
    if args.operation == "get":
        ...
        psftp_put_simple(conn, args.localfile, args.remotedir)
    else:
        logger.error("the following arguments are required:get|put")
        sys.exit(1)
