...
OS_SEC_L2_TEXT U32 PRT_LogInit(uintptr_t memBase)
{
    ...
    head_ptr = (volatile uint32_t *)(g_logMemBase + HEAD_PTR_OFFSET);
    tail_ptr = (volatile uint32_t *)(g_logMemBase + TAIL_PTR_OFFSET);
...
static OS_SEC_L2_TEXT U32 OsLog(enum OsLogLevel level, enum OsLogFacility facility, const char *str, size_t strLen)
{
    ...
    cache_flush_by_range(targetMem, strLen + sizeof(struct logHeader));
