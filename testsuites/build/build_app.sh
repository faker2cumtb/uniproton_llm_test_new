...
RWFLAG=("UniProton_test_posix_rwflag_interface")
...
if $sim_flag; then
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DSIM:STRING="_SIM_" -DLIB_RUN_TYPE:STRING=$LIB_RUN_TYPE -DCPU_TYPE:STRING="m4"
else
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DLIB_RUN_TYPE:STRING=$LIB_RUN_TYPE -DCPU_TYPE:STRING="m4"
fi
pushd $TMP_DIR
make $APP --trace
popd
if [[ $APP == "UniProton_test_posix_math_interface" ]]; then
    all_math_app=$(ls $TMP_DIR/UniProton_test_posix_math_*)
    ...
    all_exit_app=$(ls $TMP_DIR/UniProton_test_posix_exit_*)
